...
public class HorizontalDirectionalRedstoneEmitterPowerMinecartEntity
    extends RedstoneBlockMinecartEntity {
  public HorizontalDirectionalRedstoneEmitterPowerMinecartEntity(
      EntityType<? extends RedstoneBlockMinecartEntity> entityType, Level world) {
    super(entityType, world);
  }

  public HorizontalDirectionalRedstoneEmitterPowerMinecartEntity(
      EntityType<? extends RedstoneBlockMinecartEntity> entityType,
      Level world,
      double x,
      double y,
      double z,
            ...
            blockState -> {
              if (blockState.hasProperty(HorizontalDirectionalBlock.FACING)) {
                this.setCustomDisplayBlockState(
                    Optional.of(
                        blockState.setValue(HorizontalDirectionalBlock.FACING, Direction.WEST)));
              }
            });
  }
  ...
  public void tick() {
    ...
    if (blockState.hasProperty(DiodeBlock.POWERED)
        && getPower(this.level(), this.blockPosition(), blockState) > 0) {
      this.setCustomDisplayBlockState(
          Optional.of(getDefaultDisplayBlockState().setValue(DiodeBlock.POWERED, true)));
      this.updateNeighbors(this.level(), this.blockPosition(), blockState.getBlock());
    } else {
      this.setCustomDisplayBlockState(
          Optional.of(getDefaultDisplayBlockState().setValue(DiodeBlock.POWERED, false)));
      this.updateNeighbors(this.level(), this.blockPosition(), blockState.getBlock());
    }
  }
  ...
  protected int getPower(Level world, BlockPos pos, BlockState state) {
    ...
    int i =
        Math.max(world.getDirectSignal(blockPos, direction), world.getSignal(blockPos, direction));
    return i > 0 ? 15 : 0;
  }
}
