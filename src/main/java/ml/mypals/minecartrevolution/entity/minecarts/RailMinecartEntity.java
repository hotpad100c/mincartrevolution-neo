...
public class RailMinecartEntity extends VariantBlockMinecartEntity {
    ...
    public void tick()
    {
        ...
        if(this.getPassengers().isEmpty() && this.getStackCooldown() == 0){
            ...
            List<AbstractMinecart> minecarts = this.level().getEntitiesOfClass(AbstractMinecart.class,  hitbox,entity -> !(entity == this) && entity.getVehicle() == null);
        ...
        if (this.getVehicle() == null && !this.level().isClientSide()) {
            ...
            if (layers > 24) {
                ...
                if (this.getRandom().nextInt(60) == 0) {
                    Vec3 scatterDir = new Vec3(this.getRandom().nextDouble() - 0.5, 0.5, this.getRandom().nextDouble() - 0.5).normalize();

                    current = this;
                    while (current != null) {
                        Entity next = current.getPassengers().isEmpty() ? null : current.getPassengers().getFirst();
                        current.stopRiding();
                    ...
                    for (ServerPlayer player : players) {
                        ...
                        //TODO umm not cool enough packetDistributor.sendToPlayer(player, new BabelScramblePacket());
                    }
                }
            }
        }
