  ...
  public void tick() {
    ...
    if (!this.level().isClientSide()) {
      ...
      if (this.sourceJukeBoxMinecartEntity == null
          || this.sourceJukeBoxMinecartEntity.isRemoved()
          || this.sourceJukeBoxMinecartEntity.getDisc().isEmpty()
          || this.sourceJukeBoxMinecartEntity.getPowerStrength(null, null) == 0) {
      ...
      if (needsRecalculation) {
        ...
        List<JukeboxMinecartEntity> jukeboxes =
            this.level()
                .getEntitiesOfClass(JukeboxMinecartEntity.class, this.getBoundingBox().inflate(32));
        ...
        if (bestDistance > 1) {
          List<AmethystMinecartEntity> amethysts =
              this.level()
                  .getEntitiesOfClass(
                      AmethystMinecartEntity.class, this.getBoundingBox().inflate(32));
          for (AmethystMinecartEntity amethyst : amethysts) {
            if (amethyst != this && amethyst.sourceJukeBoxMinecartEntity != null) {
              ...
              if (!src.isRemoved()
                  && !src.getDisc().isEmpty()
                  && src.getPowerStrength(null, null) > 0) {
