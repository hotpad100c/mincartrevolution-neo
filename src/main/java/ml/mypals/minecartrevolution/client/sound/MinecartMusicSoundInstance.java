...
public class MinecartMusicSoundInstance extends AbstractTickableSoundInstance {
    ...
    public MinecartMusicSoundInstance(AbstractMinecart minecart, Identifier location) {
        super(SoundEvent.createVariableRangeEvent(location), SoundSource.RECORDS, net.minecraft.util.RandomSource.create());
    ...
    @Override
    public void tick() {
        if (this.minecart.isRemoved() || !this.minecart.hasPassenger(e -> e == net.minecraft.client.Minecraft.getInstance().player)) {
            this.stop();
            return;
        }
