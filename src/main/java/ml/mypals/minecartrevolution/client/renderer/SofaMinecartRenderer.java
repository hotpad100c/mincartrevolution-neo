...
public class SofaMinecartRenderer
    extends AbstractMinecartRenderer<SofaMinecartEntity, SofaMinecartRenderState> {
  ...
  public void extractRenderState(
      @NonNull SofaMinecartEntity entity,
      @NonNull SofaMinecartRenderState state,
      float partialTicks) {
  ...
  public static void renderSofaWithBlockTexture(
    ...
    List<BakedQuad> sofaModel =
        Objects.requireNonNull(
                Minecraft.getInstance().getModelManager().getStandaloneModel(SOFA_MODEL_KEY))
            .getAll();
    if (sofaModel.isEmpty() || blockState.isAir()) {
      return;
    }

    renderModelWithTexture(pose, buffer, sofaModel, packedLight, packedOverlay, blockTexture);
  }
  ...
  private static void renderFace(
    ...
    for (int i = 0; i < 4; i++) {
      ...
      buffer
          .addVertex(pose.pose(), face.position(i).x(), face.position(i).y(), face.position(i).z())
