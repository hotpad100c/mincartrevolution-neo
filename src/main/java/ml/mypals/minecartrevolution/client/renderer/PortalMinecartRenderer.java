...
public class PortalMinecartRenderer extends AbstractMinecartRenderer<PortalMinecartEntity, PortalMinecartRenderState> {
    ...
    @Override
    public void extractRenderState(@NonNull PortalMinecartEntity entity, @NonNull PortalMinecartRenderState state, float partialTicks) {
    ...
    protected void renderPlane(BlockState portalBlock, PoseStack poseStack, SubmitNodeCollector submitNodeCollector, int light) {
        TextureAtlasSprite sprite = Minecraft.getInstance().getModelManager().getBlockStateModelSet().
                get(portalBlock).particleMaterial().sprite();
        submitNodeCollector.submitCustomGeometry(poseStack, RenderTypes.entityTranslucent(sprite.atlasLocation()), (pose, buffer) -> {


            float height = 0.6f;
            buffer.addVertex(pose, -2.1f, height, 0.5f).setColor(-1).setUv(sprite.getU0(), sprite.getV0()).setOverlay(OverlayTexture.NO_OVERLAY).setLight(light).setNormal(pose, 0.0f, 1.0f, 0.0f);
            buffer.addVertex(pose, -2.1f, height, 1.9f).setColor(-1).setUv(sprite.getU0(), sprite.getV1()).setOverlay(OverlayTexture.NO_OVERLAY).setLight(light).setNormal(pose, 0.0f, 1.0f, 0.0f);
            buffer.addVertex(pose, -0.3f, height, 1.9f).setColor(-1).setUv(sprite.getU1(), sprite.getV1()).setOverlay(OverlayTexture.NO_OVERLAY).setLight(light).setNormal(pose, 0.0f, 1.0f, 0.0f);
            buffer.addVertex(pose, -0.3f, height, 0.5f).setColor(-1).setUv(sprite.getU1(), sprite.getV0()).setOverlay(OverlayTexture.NO_OVERLAY).setLight(light).setNormal(pose, 0.0f, 1.0f, 0.0f);
        });
    }

    @Override
    protected void submitMinecartContents(PortalMinecartRenderState state,
