...
public class ScaffoldMinecartRenderer
    extends AbstractMinecartRenderer<ScaffoldMinecartEntity, ScaffoldMinecartRenderState> {
  ...
  public void extractRenderState(
      @NonNull ScaffoldMinecartEntity entity,
      @NonNull ScaffoldMinecartRenderState state,
      float partialTick) {
    ...
    for (int i = 0; i < ScaffoldMinecartEntity.SLOT_COUNT; i++) {
      ...
      cluster.extractItemGroupRenderState(
          entity, stack, Minecraft.getInstance().getItemModelResolver());
  ...
  private void renderPile(
      ...
      int lightCoords,
      ItemClusterRenderState[] clusters,
      int offset,
      int count,
      float baseY) {
    long pileSeed = 0L;
    ...
    for (int i = offset; i < offset + count; i++) {
      ...
      submitMultipleFromCount(
          ...
          cluster,
          random,
          stackState.getModelBoundingBox());
