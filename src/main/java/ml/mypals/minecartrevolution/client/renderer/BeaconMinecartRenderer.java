...
public class BeaconMinecartRenderer
    extends AbstractMinecartRenderer<BeaconMinecartEntity, BeaconMinecartRenderState> {
  ...
  public void extractRenderState(
      BeaconMinecartEntity entity, BeaconMinecartRenderState state, float partialTicks) {
    super.extractRenderState(entity, state, partialTicks);
    state.animationTime =
        entity.level() != null
            ? Math.floorMod(entity.level().getGameTime(), 40) + partialTicks
            : 0.0F;
    ...
    state.sections =
        entity.getBeamSections().stream()
            .map(
                section ->
                    new BeaconMinecartRenderState.Section(section.getColor(), section.getHeight()))
            .toList();
  }

  @Override
  protected void submitMinecartContents(
      BeaconMinecartRenderState state,
    ...
    submitNodeCollector.submitCustomGeometry(
        ...
        (pose, buffer) ->
            renderPart(
                ...
                0.0F,
                1.0F,
                height * (0.5F / solidBeamRadius) + texVOff - 1.0F,
                texVOff - 1.0F));
    poseStack.popPose();
