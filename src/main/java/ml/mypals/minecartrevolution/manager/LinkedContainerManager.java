...
public class LinkedContainerManager {
    ...
    public void load(MinecraftServer server) {
        ...
        File file = server.getWorldPath(LevelResource.ROOT).resolve(MinecartRevolution.MODID).resolve(FILE_NAME).toFile();
        if (!file.exists()) {
            return;
        }
        ...
        try (FileReader reader = new FileReader(file)) {
            ...
            data.forEach((key, snbt) -> {
                ...
                try {
                    ...
                    input.read("Items", ItemContainerContents.CODEC).ifPresent(contents -> {
    ...
    public void save(MinecraftServer server) {
        ...
        containers.forEach((key, container) -> {
            try {
                ...
                ItemContainerContents.CODEC.encodeStart(ops, contents)
                        .resultOrPartial(_ -> {})
